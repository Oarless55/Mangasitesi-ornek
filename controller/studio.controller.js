...
class StudioController {
  ...
  delete() {
    return {
      ...
      category: async (_id) => {
        if (!UserController.isMod(this.user)) {
          throw new ForbiddenError('Bu işlem için yönetici yetkisi gereklidir')
        }
        const category = await Category.findById(_id)
        if (!category) {
          throw new ForbiddenError('Kategori bulunamadı')
        }
        // Remove from stories
  ...
  async createCategory(name) {
    ...
    if (!name) {
      throw new ApolloError('Kategori adı boş olamaz', 'NOTIFY')
    }
    const check = await Category.findOne({ name })
    if (check) {
      throw new ApolloError('Bu kategori zaten mevcut', 'NOTIFY')
    }
