<template>
  <div>
          ...
          <a-col span="17" offset="1">
            <a-form-model-item ref="nameExtend" class="chapter-name" prop="nameExtend">
              <a-input
                ...
                placeholder="Bölüm Ek Adı (Örn: Özel Bölüm)"
                size="large"
                :max-length="70"
              />
            </a-form-model-item>
        ...
        <a-row>
          <a-col span="6">
            <a-form-model-item ref="avatar" prop="avatar">
              ...
              <template slot="extra">
                <small>
                  - Bölüm kapağı yüklemek için tıklayın.
                  <br>
                  - Resim kırpma aracı otomatik açılacaktır!
                </small>
              </template>
            </a-form-model-item>
          </a-col>
          <a-col span="17" offset="1">
            <a-form-model-item ref="note" class="chapter-name" prop="note">
              <a-input
                v-model="form.note"
                placeholder="Bölüm Notu / Açıklama"
                size="large"
                :max-length="70"
              />
              <template slot="extra">
                <small>
                  - Uyarılar veya notlar bölümün başında görünür.
                </small>
              </template>
            </a-form-model-item>
          </a-col>
        ...
        <div class="chapter-content">
          <div class="comic-tool-bar">
            <div class="comic-tool-label">
              İçerik ({{ form.content.length }} Resim)
            </div>
            <div v-if="form.content.length && !uploading">
              ...
              >
                Sil Seçilenleri ({{ imageSelected.length }})
              </a-button>
              <a-button
                size="small"
                type="primary"
    ...
    >
      ...
      <small>
        - Bölümünüz için bir yayınlama zamanı ayarlayın.
        <br>
        - Gerçek yayınlanma süresi planlanandan 10 dakika kadar gecikebilir!
      </small>
    </a-modal>
  </div>
</template>
...
export default {
  ...
  data () {
    return {
      ...
      rules: {
        name: [
          { required: true, message: 'Bölüm adını girmek zorunludur', trigger: 'blur' }
        ]
      },
      upload: {
  ...
  methods: {
      ...
      this.$refs.cropper.getCroppedCanvas().toBlob((data) => {
        if (!data) {
          ...
          this.$axios
            ...
            .catch(() => {
              this.$message.error('Yükleme başarısız')
            })
            .finally(() => {
              this.isLoadingImage = false
              this.visible = false
              this.$message.success('Kırpma ve yükleme başarılı')
    ...
    onSubmit () {
      this.$refs.ruleForm.validate((valid) => {
        if (valid) {
          ...
          this.$message.error('Lütfen bilgileri kontrol edin')
          return false
        }
      })
    },
    ...
    schedulerChapter () {
      if (!this.publishTime.time || !this.publishTime.date) {
        return this.$message.error('Yayınlama süresi geçersiz')
      }
      ...
      if (publishTime < Date.now()) {
        return this.$message.error('Yayınlama tarihi geçmişte olamaz')
      }
    ...
    async publishChapter (scheduler) {
      ...
      try {
        ...
        if (this.form._id) {
          this.$message.success('Başarıyla güncellendi')
        } else {
          this.$message.success('Bölüm başarıyla eklendi')
    ...
    async uploadImage (file, index) {
      ...
      try {
        ...
        this.$message.error('Yükleme sırasında hata oluştu')
      }
    },
    ...
    showConfirm () {
      this.$confirm({
        ...
        content: 'Bölümün içindeki tüm içerikler silinecek!',
        onOk: () => {
