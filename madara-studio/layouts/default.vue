  ...
  <a-layout id="components-layout-demo-custom-trigger">
    <a-layout-sider>
      ...
      <a-menu theme="dark" mode="inline" :default-selected-keys="['1']">
        ...
        <a-menu-item v-if="['mod', 'admin'].includes(user.role)" key="2" @click="$route.name !== 'categories' && $router.push({ name: 'categories' })">
          <a-icon type="build" />
          <span>Kategoriler</span>
        </a-menu-item>
        <a-menu-item v-if="['mod', 'admin'].includes(user.role)" key="3" @click="$route.name !== 'members' && $router.push({ name: 'members' })">
          <a-icon type="user" />
          <span>Üyeler</span>
        </a-menu-item>
        <a-menu-item key="5" @click="$route.name !== 'bot' && $router.push({ name: 'bot' })">
